...
class DarcyNet2D_BCs_Y:
    # Initialize the class
    def __init__(self, X_y, Y_y, X_u, Y_u, X_f, Y_f, 
                 X_ubD, Y_ubD, X_ubN, Y_ubN, normal_vec,
                 layers_y, layers_u, lb, ub):
     
        self.lb = lb
        self.ub = ub
        X_y = (X_y - lb) - 0.5*(ub - lb)
        X_u = (X_u - lb) - 0.5*(ub - lb)
        X_f = (X_f - lb) - 0.5*(ub - lb)
        X_ubD = (X_ubD - lb) - 0.5*(ub - lb)
        X_ubN = (X_ubN - lb) - 0.5*(ub - lb)
        ...
        # Initialize network weights and biases  
        self.weights_y, self.biases_y = self.initialize_NN(layers_y)        
        self.weights_u, self.biases_u = self.initialize_NN(layers_u)
        ...
        # Define placeholders and computational graph
        self.x1_y_tf = tf.placeholder(tf.float32, shape=(None, self.x1_y.shape[1]))
        self.x2_y_tf = tf.placeholder(tf.float32, shape=(None, self.x2_y.shape[1]))        
        self.Yy_tf = tf.placeholder(tf.float32, shape=(None, self.Y_y.shape[1]))
        ...
        # Evaluate prediction
        self.y_pred = self.net_y(self.x1_y_tf, self.x2_y_tf)
        self.u_pred = self.net_u(self.x1_u_tf, self.x2_u_tf)
        self.f_pred = self.net_f(self.x1_f_tf, self.x2_f_tf)
        ...
        # Evaluate loss
        self.loss = tf.losses.mean_squared_error(self.Yy_tf, self.y_pred) + \
                    tf.losses.mean_squared_error(self.Yu_tf, self.u_pred) + \
                    tf.losses.mean_squared_error(self.Yf_tf, self.f_pred) + \
                    tf.losses.mean_squared_error(self.YubD_tf, self.ubD_pred) + \
                    tf.losses.mean_squared_error(self.YubN_tf, self.ubN_pred)
    ...
    def net_f(self, x1, x2):
        ...
        nabla_u = tf.gradients(u_x1, x1)[0]+\
            tf.gradients(u_x2, x2)[0]
        #nabla_u = tf.linalg.trace(tf.hessians(u, [x1,x2])[0])
        f = k*(nabla_u)+k*grady_dot_gradu
        return f
    ...
    def net_ubN(self, x1, x2, n):
        u = self.net_u(x1, x2)
        y = self.net_y(x1, x2)
        g_1 = tf.exp(y)*tf.gradients(u, x1)[0]
        g_2 = tf.exp(y)*tf.gradients(u, x2)[0]
        g = g_1*n[:,0:1] + g_2*n[:,1:2]
        return g
    ...
    def train(self): 
        ...
        # Define a dictionary for associating placeholders with data
        tf_dict = {self.x1_y_tf: self.x1_y, self.x2_y_tf: self.x2_y, self.Yy_tf: self.Y_y,
                   self.x1_u_tf: self.x1_u, self.x2_u_tf: self.x2_u, self.Yu_tf: self.Y_u, 
                   self.x1_f_tf: self.x1_f, self.x2_f_tf: self.x2_f, self.Yf_tf: self.Y_f,
                   self.x1_ubD_tf: self.x1_ubD, self.x2_ubD_tf: self.x2_ubD, self.YubD_tf: self.Y_ubD,
                   self.x1_ubN_tf: self.x1_ubN, self.x2_ubN_tf: self.x2_ubN, self.YubN_tf: self.Y_ubN,
    ...
    def predict_y(self, X_star): 
        ...
        X_star = (X_star - self.lb) - 0.5*(self.ub - self.lb)
        # Predict
        tf_dict = {self.x1_y_tf: X_star[:,0:1], self.x2_y_tf: X_star[:,1:2]}    
